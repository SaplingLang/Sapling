...
public class TokenSpec {
    ...
    public void string_to_int_should_throw_InvalidType() {
        Token bad_string = new Token(Token.TokenType.IDENTIFIER, "BAD_STRING", 0, 0);
        try {
            bad_string.integer();
    ...
    public void int_to_string_should_throw_InvalidType() {
        Token bad_int = new Token(Token.TokenType.INTEGER, 1, 0, 0);
        try {
            bad_int.string();
            Assert.fail("Failed To Throw InvalidType.");
        } catch (InvalidType invalidType) {
    ...
    public void null_to_string_should_throw_InvalidType() {
        Token null_value = new Token(Token.TokenType.INTEGER, null, 0, 0);
        try {
            null_value.string();
            Assert.fail("Failed To Throw InvalidType.");
        } catch (InvalidType invalidType) {
